...
class Mastermind:
	...
	def is_correct_input(self, line_input):
		#Returns true if input match this mastermind play.
		#TODO Improve
		for l in line_input:
			if l not in self.color_dict:
				print(l+' -> wrong letter\n Colors are '+' '.join(self.color_dict))
				return False

		if len(line_input)==len(self.colors):
			return True
